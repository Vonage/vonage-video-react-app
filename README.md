...
- **Environment Variables**
  ...
  Frontend feature flags and display settings are configured in [`env.sh`](env.sh). The defaults work out of the box — edit that file only when you need to customise behaviour. See [Environment Configuration](#environment-configuration) for the full list of available options.
...
The app has two parts: a **backend** server and a **frontend** UI.

Project-level frontend configuration is defined in `env.sh`. This file is committed to the repository and contains the default/shared frontend settings used by the project.

For local configuration, use `.env` files. Create them by running:
...
Frontend local configuration is optional. The app uses the committed `env.sh` values by default. If you need to override frontend settings locally, use the `frontend/.env` file.

The backend, however, requires you to set your Vonage application ID and private key as described below.
...
| `TUNNEL_DOMAIN` | `string` | Not set | Domain name | Public tunnel domain used when testing across devices. See [Testing on Multiple Devices](#testing-on-multiple-devices) |
