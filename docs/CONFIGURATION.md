...
| `OIDC_CLIENT_ID` | if enabled — DEV defaults in `env.sh` | OIDC application client ID, used both for Bearer-token introspection (Mobile) and the `/auth/signin` → `/api/auth/callback/okta` login flow (Web) — one app registration shared by both clients. |
