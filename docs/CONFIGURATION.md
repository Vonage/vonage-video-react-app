...
| `OIDC_CLIENT_ID` | if enabled — DEV defaults in `env.sh` | OIDC application client ID used for Bearer-token introspection (Mobile). |
| `OIDC_WEB_CLIENT_ID` | if enabled — DEV defaults in `env.sh` | OIDC application client ID used for the `/auth/signin` → `/api/auth/callback/okta` login flow (Web) — separate app registration from `OIDC_CLIENT_ID`. |
