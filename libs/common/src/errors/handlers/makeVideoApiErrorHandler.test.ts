import buildVideoApiErrorHandler from '@server/helpers/errors/commonErrorsHandlers/buildVideoApiErrorHandler';

describe('buildVideoApiErrorHandler', () => {
  const safeMessage = 'This is a safe message';
  const unsafeMessage = 'This is an unhandled error';

  const handler = buildVideoApiErrorHandler(safeMessage);
  ...
  it('should correctly handle opentok errors', () => {
    const src = {
      message:
        'Unexpected response from OpenTok: "{"message":"The url structure must be valid."}"',
    };
