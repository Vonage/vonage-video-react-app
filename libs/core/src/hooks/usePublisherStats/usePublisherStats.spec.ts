...
describe('usePublisherStats', () => {
  ...
  describe('encoded resolution', () => {
    ...
    it('ignores layers that report no dimensions, which is how idle encodings arrive', async () => {
      expect.assertions(1);
      ...
      const publisher = makePublisher(
        [
          makeStatsContainer({
            video: {
              layers: [
                // An encoding that is still active but not transmitting: it keeps reporting a
                // bitrate and a frame rate, but no dimensions at all.
    ...
    it('falls back to the captured dimensions when no layer has encoded a frame yet', async () => {
      expect.assertions(1);

      const publisher = makePublisher(
        [
          makeStatsContainer({
            video: { layers: [{ encodedFrameRate: 0, bitrate: 0 }, { encodedFrameRate: 0 }] },
          }),
        ],
        { videoWidth: 1280, videoHeight: 720 }
      );
      ...
      expect(stats.resolution.toString()).toBe('1280x720');
    });

    it('falls back to the captured dimensions when no layers are reported', async () => {
      expect.assertions(1);

      const publisher = makePublisher([makeStatsContainer({ video: { layers: [] } })], {
        videoWidth: 1280,
        videoHeight: 720,
      });
