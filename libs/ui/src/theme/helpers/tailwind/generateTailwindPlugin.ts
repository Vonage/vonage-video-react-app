...
import type { VeraTypographyTokenKey, VeraTypographyVariableNamesByToken } from '../veraUI.types';
...
const veraLayoutVariableNames = {
  borderRadiusMedium: '--vera-border-radius-medium',
} as const;

const veraFontVariableNames = {
  fontFamilyPlain: '--vera-font-family-plain',
} as const;
...
const veraUI = plugin(({ addUtilities, addBase, addVariant }) => {
  ...
  addBase({
    ':root': {
${toObjectEntriesLiteral(colorVariables.root, 6)}

    // Typography and layout design tokens
  ${toObjectEntriesLiteral(typographyAndLayoutVariables, 6)}
    },
...
function generateTypographyAndLayoutVariables(args: {
  ...
  variables[veraLayoutVariableNames.borderRadiusMedium] = args.borderRadius.medium;
  variables[veraFontVariableNames.fontFamilyPlain] = args.fontFamily.plain;
...
/**
 * Converts object entries into JS object literal lines with stable indentation.
 */
function toObjectEntriesLiteral(object: Record<string, string>, indentationLength: number): string {
  const indentation = ' '.repeat(indentationLength);
  return Object.entries(object)
    .map(([key, value]) => `${indentation}${JSON.stringify(key)}: ${JSON.stringify(value)},`)
    .join('\n');
}
