...
describe('DeviceSettingsMenu Component', () => {
  ...
  beforeEach(async () => {
    ...
    ({ DeviceSettingsMenuComponent, mediaDevices$ } = await importDeviceSettingsMenu());
...
function render(
  ...
  return {
    ...appConfigContext,
    ...renderBase(ui, { wrapper: AppConfigWrapper }),
  };
}

async function importDeviceSettingsMenu() {
  const mediaDevices$ = (await import('@core/stores/devices')).default;

  const DeviceSettingsMenuComponent = (
    await vi.importActual<typeof import('./DeviceSettingsMenu')>('./DeviceSettingsMenu')
  ).default;

  return { DeviceSettingsMenuComponent, mediaDevices$ };
}
