 ...
 *
 * Hidden when ALLOW_SPATIAL_AUDIO is false.
 * Rendered as disabled with a tooltip when the browser does not support the Web Audio API.
 *
 * @returns {ReactElement | null} The SpatialAudioToggle component, or null if the feature is disabled.
 */
const SpatialAudioToggle = (): ReactElement | null => {
  ...
  if (!env.ALLOW_SPATIAL_AUDIO) {
    return null;
  }

  const isSupported = hasWebAudioSupport();

  const toggleRow = (
    <MenuItem
      onClick={isSupported ? toggleSpatialAudio : undefined}
    ...
    >
      <Box sx={{ mr: 2 }}>
        <VividIcon customSize={-5} name="headset-solid" sx={{ color: theme.colors.secondary }} />
      </Box>
    ...
    <>
      ...
      >
        {isSupported ? (
          ...
          <Tooltip title={t('spatialAudio.tooltip.unsupported')} placement="right">
            <Box component="span">{toggleRow}</Box>
          </Tooltip>
        )}
      </MenuList>
    </>
