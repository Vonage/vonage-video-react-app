...
sessionRouter.post(
  ...
  async (req: Request<{ room: string }>, res: Response) => {
    try {
      ...
      await videoService.enableCaptions(sessionId);
      res.json({ status: 200 });
    } catch (error: unknown) {
      const isCaptionsAlreadyEnabledError = (() => {
        if (error instanceof Error && error.message.includes('status code 409')) return true;
        if (!error || typeof error !== 'object') return false;

        const errorRecord = error as Record<string, unknown>;

        const hasStatusCode409 = errorRecord.statusCode === 409;
        if (hasStatusCode409) return true;

        const response = errorRecord.response;
        if (!response || typeof response !== 'object') return false;

        const responseRecord = response as Record<string, unknown>;
        return responseRecord.status === 409;
      })();

      if (isCaptionsAlreadyEnabledError) {
        res.json({ status: 200 });
        return;
      }
