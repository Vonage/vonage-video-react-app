...
function main() {
  ...
  printProgress('Phase 3/6: running quality gates (prettier, eslint, ts-check)...');
  ...
  printProgress('Phase 3/6: quality gates complete.');

  printProgress('Phase 4/6: building queue manifest...');
  ...
  printProgress('Phase 4/6: queue manifest ready.');
...
function printClassificationSummary(result: {
  ...
  if (excludedFiles.length > 0) {
    const groupedByReason = excludedFiles.reduce<Record<string, number>>(
      (accumulator, excludedFile) => {
...
function parseOptions(argumentsList: string[]): HarnessOptions {
  ...
  const requiresScopeValue =
    scopeType === 'folder' || scopeType === 'file' || scopeType === 'commit';
  ...
  const coverageThreshold = parsePositiveNumber(
    parsed['coverage-threshold'] ?? '80',
    '--coverage-threshold'
  );
  const maxRetriesGlobal = parseNonNegativeInteger(
    parsed['max-retries-global'] ?? '2',
    '--max-retries-global'
  );
  ...
  const maxRetriesFile = parseNonNegativeInteger(
    parsed['max-retries-file'] ?? '2',
    '--max-retries-file'
  );
...
function printHelp() {
  ...
  console.log(
    '  --monitoring-dir <path>                     Default: _testMonitoring/codeReviewHarness'
  );
  ...
  console.log(
    '  yarn code-review:harness --scope-type working-tree --target-mode current-changes --dry-run'
  );
  console.log(
    '  yarn code-review:harness --scope-type file --scope-value backend/routes/video/video.ts --target-mode whole-file --dry-run'
  );
  console.log(
    '  yarn code-review:harness --scope-type commit --scope-value <sha> --target-mode current-changes --ai-command "copilot"'
  );
}
...
function resolveScope(options: HarnessOptions): ResolvedScope {
  ...
  const candidateFiles = (() => {
    ...
    if (options.scopeType === 'file') {
      const normalizedFilePath = normalizeWorkspacePath(
        options.scopeValue as string,
        workspaceRoot
      );
...
function getExclusionReason(classification: FileClassification): string | null {
  ...
  if (
    /\b(node_modules|coverage|dist|docs|reports|tmp|playwright-report|test-results)\b/.test(
      filePath
    )
  ) {
...
function runScopeQualityGates(args: {
  ...
  const impactedProjects = uniqueStrings(
    testableFiles
      ...
      .filter(
        (projectName): projectName is Exclude<ProjectName, 'unknown'> => projectName !== 'unknown'
      )
  );
...
function processFilesSequentially(args: {
  ...
  for (const [fileIndex, fileClassification] of testableFiles.entries()) {
    ...
    printProgress(
      `Phase 5/6: processing file ${fileIndex + 1}/${testableFiles.length}: ${filePath}`
    );

    if (
      options.targetMode === 'current-changes' &&
      !resolvedScope.changedFiles.includes(filePath)
    ) {
      fileResults.push({
        filePath,
        status: 'skipped',
    ...
    if (
      baselineCoverage.coveragePercentage !== null &&
      baselineCoverage.coveragePercentage >= options.coverageThreshold
    ) {
      fileResults.push({
        filePath,
        status: 'skipped',
...
function runMetadataStep(args: {
  ...
  runCommandCapture(
    aiCommandLine,
    `AI metadata command failed for ${fileClassification.filePath}.`,
    {
      liveOutput: true,
    }
  );
...
function runAutofixStep(args: {
  ...
  runCommandCapture(
    aiAutofixCommandLine,
    `AI autofix command failed for ${fileClassification.filePath}.`,
    {
      liveOutput: true,
    }
  );
...
function buildGlobalInstructionScore(
  fileResults: Array<{
    filePath: string;
    instructionValidation?: InstructionValidationEvidence;
  }>
  ...
  score: { passedRules: number; totalRules: number; percentage: number };
} {
  const ruleScoresByRuleId = new Map<
    string,
    {
      ruleId: string;
      description: string;
  ...
  const reviewedRuleMap = new Map<string, ReviewedRule>();

  for (const result of fileResults) {
    const evidence = result.instructionValidation;
    if (!evidence) continue;

    for (const reviewedRule of evidence.reviewedRules) {
      if (!reviewedRuleMap.has(reviewedRule.ruleId)) {
        reviewedRuleMap.set(reviewedRule.ruleId, reviewedRule);
      }
    }

    for (const violation of evidence.violations) {
      const existing = ruleScoresByRuleId.get(violation.ruleId) ?? {
        ruleId: violation.ruleId,
        description: violation.description,
        instructionsSource: violation.instructionsSource,
        violations: [],
      };
      existing.violations.push(violation);
      ruleScoresByRuleId.set(violation.ruleId, existing);
    }
  }

  const reviewedRules = Array.from(reviewedRuleMap.values());

  const ruleScores = reviewedRules.map((reviewedRule) => {
    const ruleEntry = ruleScoresByRuleId.get(reviewedRule.ruleId);
    const ruleViolations = ruleEntry?.violations ?? [];
  ...
  const percentage =
    totalRules === 0 ? 100 : Number(((passedRules / totalRules) * 100).toFixed(2));
  const totalViolationCount = ruleScores.reduce(
    (sum, ruleScore) => sum + ruleScore.violationCount,
    0
  );
  ...
  return {
    ...
    ruleScores,
    totalViolationCount,
    score: { passedRules, totalRules, percentage },
  };
}
...
function emitFinalSummary(args: {
  ...
  const globalInstructionScore = buildGlobalInstructionScore(fileResults);
...
function getReviewedRules(filesToCheck: string[]): ReviewedRule[] {
  ...
  const includesTestFile = filesToCheck.some((filePath) =>
  ...
  return [
    {
      ...
      description:
        'Do not use React.Suspense or Suspense directly; use SuspenseBoundary/Suspense$.',
